...
contract CostModelDynamicLevel is ICostModel {
    ...
    /// @dev Returns the area under the curve between the `intervalLowPoint_` and `intervalHighPoint_`, scaled up by wad^3.
    ...
    /// @dev Returns the cost factor (y-coordinate) of the point where the utilization equals the given `utilization_` (x-coordinate).
    ...
    function _computeNewCostFactorInOptimalZone(uint256 utilization_, uint256 timeDelta_)
        internal
        view
        returns (uint256)
    {
        ...
        if (utilization_ >= uOpt) {
            // Cost factor increases with `timeDelta` and `utilization - uOpt`, but with a ceiling set at `costFactorAtFullUtilization`.
            return _min(
                currentCostFactorInOptimalZone_ + optimalZoneRate.mulWadUp((utilization_ - uOpt) * timeDelta_),
                costFactorAtFullUtilization
            );
        } else {
            // Cost factor decreases with `timeDelta` and `utilization - uOpt`, but with a floor set at `costFactorAtZeroUtilization`.
            uint256 delta_ = optimalZoneRate.mulWadUp((uOpt - utilization_) * timeDelta_);
            if (delta_ > currentCostFactorInOptimalZone_ - costFactorAtZeroUtilization) {
                return costFactorAtZeroUtilization;
            } else {
                return currentCostFactorInOptimalZone_ - delta_;
            }
        }
    }

    /// @dev Returns the  values of the dynamically updated storage variables, `costFactorInOptimalZone` and `lastUpdateTime`.
